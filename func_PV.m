...
%   size_PV: PV plant size [kW] positive scalar

% Output
%   P_pv: Power output of PV plant [kW] column vec

% Parameters
I_ref = 1000; % reference insolation [W/m2]
Pdc_ref = size_PV; % nominal power of PV plant [kW]
...
P_dc = ((unit_ratio*I)/I_ref*Pdc_ref).*(1+gamma*(T-T_ref)); % [kW] dc output
P_pv = eta*P_dc; %[kW] ac output to grid
